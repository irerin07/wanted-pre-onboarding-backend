...
- 채용 상세 페이지 조회시 동일한 회사의 채용 공고 id 리스트를 어떻게 할 것인가 고민
- 검색 기능 구현 방향 이슈

## 이슈 처리 과정

- 채용 상세 페이지 조회시 해당 회사의 다른 채용 공고를 조회하는 기능을 구현했다. 처음 사용한 방법은 채용 공고 데이터를 가져와 그 안에서 회사 id를 뽑아낸 뒤, 다시 한 번 채용 공고 DB를 조회하여 동일한 회사 id를 가진 채용 공고의 id를 가지고 오는 것이었다.
- 그런데 문득 생각해보니 이럴 필요 없이 채용 상세 페이지 조회시 채용공고 id와 함께 회사 id를 넘겨 받으면 채용 공고 데이터에서 뽑아 올 필요 없이 바로 사용할 수 있겠다 싶어서 Path Variable로 받도록 변경 했다.
- 조금 시간이 지난 후에 다시 생각해보니 Path Variable은 특정 리소스의 식별을 하기 위함인데 지금 내가 사용하듯이 사용하면 그 취지에 어긋난다는 생각이 들었다.
    - 그렇다고 이걸 Query Parameter로 받자니 리소스의 필터링이라 보기도 애매하다고 느꼈다.
- 결국 Path Variable로 받던 회사 id를 다시 삭제하여 기존 방법 그대로 사용하기로 했다.
- 그런데 분명 더 쉽고 효율적인 방법이 있을 것 같은데…
